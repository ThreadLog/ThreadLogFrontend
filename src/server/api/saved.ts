...
export async function saveJob(ctx: HandlerContext) {
  ...
  const existing = await prisma.savedJob.findUnique({ where: { jobSeekerId_jobId: { jobSeekerId: user.id, jobId } } });
  if (existing) throw new AppError("You have already saved this job", 400);

  await prisma.savedJob.create({ data: { jobSeekerId: user.id, jobId } });
...
export async function unsaveJob(ctx: HandlerContext) {
  ...
  await prisma.savedJob.delete({ where: { jobSeekerId_jobId: { jobSeekerId: user.id, jobId } } });
...
export async function saveCompany(ctx: HandlerContext) {
  ...
  const company = await prisma.companyRecruiter.findUnique({ where: { id: companyId } });
  if (!company) throw new AppError("Company not found", 404);

  const existing = await prisma.savedCompany.findUnique({ where: { jobSeekerId_companyId: { jobSeekerId: user.id, companyId } } });
  if (existing) throw new AppError("You have already saved this company", 400);

  await prisma.savedCompany.create({ data: { jobSeekerId: user.id, companyId } });
...
export async function unsaveCompany(ctx: HandlerContext) {
  ...
  await prisma.savedCompany.delete({ where: { jobSeekerId_companyId: { jobSeekerId: user.id, companyId } } });
